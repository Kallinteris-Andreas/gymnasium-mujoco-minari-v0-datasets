...
for algorithm in args.result_directory:
    returns = np.average(
        np.array(
            [np.load(f"models/{algorithm}/run_{run}/evaluations.npz")["results"][: steps.size] for run in range(args.runs)]
        ),
        axis=2,
    )
    returns_len = np.average(
        np.array(
            [np.load(f"models/{algorithm}/run_{run}/evaluations.npz")["ep_lengths"][: steps.size] for run in range(args.runs)]
        ),
        axis=2,
    )
